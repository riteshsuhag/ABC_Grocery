...
from pages.common_functions.app_functions import title_image, plot_image_121, prediction_input_ui
import requests
...
We now have a model object, and a the required pre-processing steps to use this model.  When this is ready to launch we can aggregate the neccessary customer information and pass it through, obtaining customer loyalty scores for new customer.

Based upon this, we can work with the client to discuss when to calculate the customer loyalty scores and how it'll benefit them in the future.

**In the meantime client can use the UI below to find the customer loyalty score for a customer.** The model is deployed using API. The API is created using FastAPI which is hosted on heroku. This serves as a backend server where models are loaded and served to make real time predictions through HTTP requests.
...
# getting the parameters from the user and saving it in json format for api -
params = {
...
prediction_output = requests.post("__address_of_the_API__",json=params)

# cleaning the output of API - 
prediction_output = prediction_output.json()

# printing the output -
print(f"Customer Loyalty Score = {round(prediction_output["Loyalty Score"], 3)}")

```

#### Enter the values below to get the prediction - 


""", unsafe_allow_html=True)
st.write("")

check_input, distance_from_store, gender_M, credit_score, total_sales, total_items, product_area_count, transaction_count = prediction_input_ui()
...
if check_input:
    ...
    prediction_output = requests.post("https://ml-api-ritesh.herokuapp.com/get_loyalty_score/",json=params)

    prediction_output = prediction_output.json()
    
    col1, col2, col3 = st.columns((1,1.4,1))
    col2.markdown(f"""### Customer Loyalty Score = {round(prediction_output["Loyalty Score"], 3)}""")

col1, col2, col3 = st.columns((1,9,1))
col2.info("""Information on how to create the API can be found in the 'Deploying ML Model' section of the navigation tab.""")
